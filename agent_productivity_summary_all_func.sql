...
begin
    ...
    SQL := 'SELECT login."SummaryDate"                                                                                                        as summary_date,
       ...
       COALESCE(inbound_hold.total_count, 0)                                                                                      as inbound_hold_total_count,
       TO_CHAR(
               (
                   COALESCE(inbound_hold.total_time, 0) || '' second''
                   )::interval,
       ...
       TO_CHAR(
               (
                           ...
                           (COALESCE(outbound_acw.total_time, 0) +
                            COALESCE(outbound_connected.total_time, 0) + COALESCE(outbound_dialed.total_time, 0)) || '' second''
                   )::interval,
               '' HH24:MI:SS ''
       ...
       TO_CHAR(
               (
                           COALESCE(outbound_dialed.total_time, 0)   || '' second''
                   )::interval,
               '' HH24:MI:SS ''
           )                                                                                                                      as outbound_dialed_total_time,
       TO_CHAR(
               (COALESCE(outbound_dialed.total_time, 0) /
		COALESCE(outbound_dialed.total_count, 1) || '' second'' )::interval,
               '' HH24:MI:SS ''
...
FROM ((select "SummaryDate"::date,
     ...
     on login."SummaryDate"::date = resource_login_same_day."createdAt"::date and
      ...
      (select "createdAt"::date,
             ...
             "CompanyId",
             "ResourceId"     as agent,
             max("createdAt") as logout_time
      from "DB_RES_ResourceStatusChangeInfos"
      where "Reason" = ''UnRegister''
      group by "createdAt"::date, "TenantId", "CompanyId", "ResourceId"
      order by logout_time
     ) as resource_logout_same_day
     on login."SummaryDate"::date = resource_logout_same_day."createdAt"::date and
        login."Tenant"::integer = resource_logout_same_day."TenantId" and
        login."Company"::integer = resource_logout_same_day."CompanyId" and
        login.agent::integer = resource_logout_same_day.agent	
	 left outer join
     ...
     (select "SummaryDate"::date,
      ...
      where "WindowName" = ''CONNECTED''
          and "Param2" = ''CALLinbound''
                and "SummaryDate" >= ''' || from_date || '''
           and "SummaryDate" <= ''' || to_date || '''
      group by "SummaryDate"::date, "Tenant", "Company") as inbound_connected_full
     on login."SummaryDate" = inbound_connected_full."SummaryDate" and login."Tenant" = inbound_connected_full."Tenant" and
        login."Company" = inbound_connected_full."Company"
         left outer join
     (select "SummaryDate"::date,
             "Tenant",
             "Company",
     ...
     (select "SummaryDate"::date,
      ...
      where "WindowName" = ''CONNECTED''
          and "Param2" = ''CALLoutbound''
                and "SummaryDate" >= ''' || from_date || '''
           and "SummaryDate" <= ''' || to_date || '''
      group by "SummaryDate"::date, "Tenant", "Company") as outbound_connected_full
     on login."SummaryDate" = outbound_connected_full."SummaryDate" and login."Tenant" = outbound_connected_full."Tenant" and
        login."Company" = outbound_connected_full."Company"
        left outer join
    (select "SummaryDate"::date,
             "Tenant",
             "Company",
     ...
     on login."SummaryDate" = outbound_dialed."SummaryDate" and login."Tenant" = outbound_dialed."Tenant" and
        login."Company" = outbound_dialed."Company" and login.agent = outbound_dialed.agent
         left outer join
     (select "SummaryDate"::date,
             "Tenant",
             "Company",
...
ALTER FUNCTION public.agent_productivity_summary_all(timestamp with time zone, timestamp with time zone)
    OWNER TO duo;
