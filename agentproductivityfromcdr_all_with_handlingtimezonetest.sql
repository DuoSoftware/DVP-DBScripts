-- FUNCTION: public.agentproductivityfromcdr_all_with_handlingtimezonetest(timestamp with time zone, timestamp with time zone)

-- DROP FUNCTION IF EXISTS public.agentproductivityfromcdr_all_with_handlingtimezonetest(timestamp with time zone, timestamp with time zone);

CREATE OR REPLACE FUNCTION public.agentproductivityfromcdr_all_with_handlingtimezonetest(
...
BEGIN
    SQL := format(
        ...
        FROM acw_times
        GROUP BY local_created_date,agent,TenantId, CompanyId
        ),
        break_times AS (
            SELECT 
                "ResourceId",
            ...
            GROUP BY "SummaryDate","ResourceId","TenantId","CompanyId"
        ),
        RankedEvents AS (
            SELECT
                "ResourceId",
            ...
            SELECT
                ...
                subquery."CompanyId"
            FROM (
                SELECT "ResourceId", "TenantId", "CompanyId"
                FROM public."DB_RES_ResourceStatusChangeInfos"
                ...
                GROUP BY "ResourceId", "TenantId", "CompanyId"
            ) AS subquery
        ),
        ...
        TotalDurations AS (
            ...
            FROM Durations
            GROUP BY session_end_date, "ResourceId", "TenantId", "CompanyId"
        ),
        ...
        Filtered_Inbound AS (
         ...
         TotalDurationsInbounds AS (
            ...
            GROUP BY  "createdAt2"::date,"ResourceId","TenantId", "CompanyId"
        ),
   ...
   ORDER BY main_query."createdAt";',
        ...
        to_date,   -- 13: Durations COALESCE NextCreatedAt (1st occurrence)
        to_date,   -- 14: Durations COALESCE NextCreatedAt (2nd occurrence)
        to_date,   -- 15: Durations COALESCE NextCreatedAt (3rd occurrence)
...
ALTER FUNCTION public.agentproductivityfromcdr_all_with_handlingtimezonetest(timestamp with time zone, timestamp with time zone)
    OWNER TO duo;
